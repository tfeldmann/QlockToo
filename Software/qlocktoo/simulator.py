...
class Simulator(QtGui.QWidget):
    ...
    def paintEvent(self, e):
        ...
        qp = QtGui.QPainter()
        qp.begin(self)
        qp.fillRect(0, 0, size.width(), size.height(), QtCore.Qt.black)

        # draw letters
        qp.setFont(QtGui.QFont('Helvetica', letter_width * 0.5))
        for y, row in enumerate(self.matrix):
            for x, cell in enumerate(row):
                color = QtGui.QColor(*[255 * cell] * 3)
                qp.setPen(color)
                qp.drawText(
        ...
        # draw corners
        colors = [QtGui.QColor(*[255 * b] * 3) for b in self.corners]
